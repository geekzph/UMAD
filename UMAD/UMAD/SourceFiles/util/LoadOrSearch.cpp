...
void multiThreadsInInterAndInner(vector<shared_ptr<CMetricData> >* queryObjectsList,char *&dataType,char *&indexFileName,double maximumSearchRadius,double minimumSearchRadius,double step,char *&resultFileName,int dataSize,int cacheHeight,int maxsearchthread,int maxthreadnumberforchildren);
...
{	
	...
	output<<"Database Size"<<setiosflags(ios::right)<<setiosflags(ios::fixed)<<setw(15)<<"Radius    "<<setiosflags(ios::right)<<setiosflags(ios::fixed)<<setw(15)<<"Distance Number"<<setiosflags(ios::right)<<setiosflags(ios::fixed)<<setw(15)<<"Time  "<<setiosflags(ios::right)<<setiosflags(ios::fixed)<<setw(15)<<"Result number"<<endl;

	for(int i=0;i<runNumber;i++)
	{
		output<<dataSize<<setiosflags(ios::right)<<setiosflags(ios::fixed)<<setw(20)<<(minimumSearchRadius+step*i)<<setiosflags(ios::right)<<setiosflags(ios::fixed)<<setw(15)<<(distanceCounter[i]/(queryObjectsList->size()))<<setiosflags(ios::right)<<setiosflags(ios::fixed)<<setw(15)<<(searchTime[i])<<setiosflags(ios::right)<<setiosflags(ios::fixed)<<setw(15)<<(((double)(resultCounter[i]))/(queryObjectsList->size()))<<endl;
	}
#ifdef _WIN32
